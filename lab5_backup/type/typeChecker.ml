  ...
  | A.Var id ->
    begin
      ...
      | None -> ErrorMsg.error None ("undeclared variable `" ^ Symbol.name id ^ "'");
        raise ErrorMsg.Error
      | Some (Decl t) -> (find_type t type_env, func_env)
      | Some (Init t) -> (find_type t type_env, func_env)
    end
  | A.LDot (lval', id) ->
    begin
...
let rec tc_stms env func_env type_env s_env ast ext ret void_func reted =
  ...
  | A.If (e, s1, s2) ->
    begin
      ...
      in let new_env = S.mapi env ~f:(fun ~key:k ~data:d ->
          ...
          | Some (Init t) -> (Init t)
          | _ -> d)
      in (new_env, check_ret_and ret1 ret2 ext type_env, func_env, reted1 && reted2)
    end
  | A.While (e, s) ->
    begin
      let (_, func_env) = tc_exp env func_env type_env s_env e ext (Some A.BOOL) in
      let (_, ret', _, _) = tc_stms env func_env type_env s_env s ext ret void_func reted in
      ...
      | _ -> begin
          ...
          let new_env = S.map env
              ~f:(fun a -> match a with
                  ...
                  | _ -> a)
          in match ret with
          | VoidRet ->  (new_env, Ret t, func_env, true)
    ...
    begin
      ...
      | A.Block s ->
        (let intersect_env = S.mapi env ~f:(fun ~key:k ~data:d ->
         ...
         in (env2, check_ret_or ret1 ret2 ext type_env, func_env, reted1 || reted2))
      | _ ->
        (let (env2, ret2,func_env, reted2) =
           tc_stms env1 new_env type_env s_env s2 ext ret void_func reted in
         (env2, check_ret_or ret1 ret2 ext type_env, func_env, reted1 || reted2))
    end
  | A.Declare (decl, s) ->
    begin
      match decl with
  ...
  | A.For s ->
    let (new_env, _, func_env, _) = tc_stms env func_env type_env s_env s ext ret void_func reted
  ...
  | A.VoidReturn ->
    (let new_env = S.map env
                        ~f:(fun a -> match a with
                            ...
                            | _ -> a)
    in match ret with
    | VoidRet -> (new_env, VoidRet, func_env, true)
