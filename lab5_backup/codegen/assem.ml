...
type instr =
  ...
  | CALLRAISE
  |  QUESTION of operand * operand * operand
...
let format_operand = function
  ...
  | TEMP t ->
    ...
    | STACK ->
      (string_of_int (
        (-(int_of_string (Temp.name t))*8 ) - 48
          )) ^ "(%rbp)"
...
let rec unsafe_format = function
    ...
    (* | T ->
       ...
       "\tmovq " ^ (get_full_reg (format_operand s1)) ^ ", %r10\n" ^
       "\tmovq " ^ get_full_reg (format_operand s2) ^ ", %r11\n" ^
       "\tcmovel %r11d, " ^ get_full_reg (format_operand d) ^ "\n"
       end *)
    | GETCONTENT (s,d) ->
      begin
        match test_if_in_stack s with
    ...
    begin
      ...
      | IMM n1, _ ->
        ...
        | T ->
          "\tmovq $1, " ^ format_operand d ^ "\n" ^
          "\tmovq " ^  (get_full_reg (format_operand s1)) ^ ", %r10\n" ^
          "\t" ^ "cmpl " ^ get_short_reg (get_full_reg (format_operand s2)) ^ ", %r10d" ^ "\n" ^
          "\tjl " ^ format_label l1 ^ "\n" ^
          "\tmovq $0, " ^ get_full_reg (format_operand d) ^ "\n" ^
          format_label l1 ^ ":\n"

        | F ->

          "\tmovq $1, " ^ format_operand d ^ "\n" ^
          (* "\tmovq " ^ format_operand s1 ^ ", %r10\n" ^ *)
          "\t" ^ "cmpl " ^ get_short_reg (get_full_reg (format_operand s1)) ^ "," ^ get_short_reg (get_full_reg (format_operand s2)) ^ "" ^ "\n" ^
          "\tjge " ^ format_label l1 ^ "\n" ^
          "\tmovq $0, " ^ get_full_reg (format_operand d) ^ "\n" ^
          format_label l1 ^ ":\n"
        end
      ...
      | F ->
        ...
        "\tmovq $1, " ^ format_operand d ^ "\n" ^
        (* "\tmovq " ^ format_operand s1 ^ ", %r10\n" ^ *)
        "\t" ^ "cmpl " ^ get_short_reg (get_full_reg (format_operand s2)) ^ "," ^ get_short_reg (get_full_reg (format_operand s1)) ^ "" ^ "\n" ^
        "\tjl " ^ format_label l1 ^ "\n" ^
        "\tmovq $0, " ^ get_full_reg (format_operand d) ^ "\n" ^
        format_label l1 ^ ":\n"
   ...
   | _ ->
    (
      begin
        ...
        | IMM n1, IMM n2 ->
          "\tmovq " ^ format_operand s1 ^ ", %r11\n" ^
          "\t" ^ format_binop oper ^ " " ^ format_operand s2 ^ ", %r11\n" ^
          "\tmovq %r11, " ^ get_full_reg (format_operand d ) ^ "\n"
        | IMM n, _ ->
          begin
            match test_if_in_stack s2 with
            | T ->
              "\tmovq " ^ format_operand s2 ^ ", %r11\n" ^
              "\t" ^ format_binop oper ^ " " ^format_operand s1 ^", %r11\n" ^
              "\tmovq %r11, " ^ get_full_reg (format_operand d) ^ "\n"
            | F ->
              "\t" ^ format_binop oper ^ " " ^ format_operand s1 ^ ", " ^  get_full_reg (format_operand s2) ^ "\n" ^
              "\tmovq " ^ get_full_reg (format_operand s2) ^ ", " ^ format_operand d ^ "\n"
          end
        ...
        (* | _,IMM n ->
           ...
           | T ->
            "\tmovq " ^ format_operand s1 ^ ", %r11\n" ^
           "\t" ^ format_binop oper ^ " %r11, " ^ get_full_reg (format_operand s1) ^ "\n" ^
           "\tmovq " ^ get_full_reg (format_operand s1) ^ ", %r11\n" ^
           "\tmovq %r11, " ^ get_full_reg (format_operand d) ^ "\n"
           | F ->
           "\t" ^ format_binop oper ^ " " ^ format_operand s1 ^ ", " ^ get_full_reg (format_operand s2) ^ "\n" ^
           "\tmovq " ^ get_full_reg (format_operand s1) ^ ", " ^ format_operand d ^ "\n"
           end *)
        | _,_ ->
          begin
            match test_if_in_stack s1 with
