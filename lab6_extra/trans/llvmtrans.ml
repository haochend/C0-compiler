...
let rec trans_exp env func_env struct_env struct_size_env prev arr_env type_env prev_addr typedef_env struct_detail_env func_id inline_env= function
    ...
    begin
      ...
      | A.NEGATIVE ->
        begin
          ...
          | A.FLOAT -> (T.BINOP (trans_binop A.MINUS, T.FCONST Float.zero, exp, t1, t2, trans_ident_type_to_string "" true_type), true_type)
          | _ -> (T.BINOP (trans_binop A.MINUS, T.CONST Int32.zero, exp,t1,t2,trans_ident_type_to_string "" true_type),true_type))
        end
      | _ ->
        begin
          let t1 = Temp.createeight() in
  ...
  | A.Alloc t ->
    ...
    begin
      ...
      | A.FLOAT -> (T.MALLOC (4, "float"), (A.POINTER A.FLOAT))
      | A.STRUCT s ->
        begin
  ...
  | A.AllocArray (ty,exp)->
    ...
    begin
      ...
      | A.FLOAT ->
        (T.ALLOCARRY (4, exp1 ,element_type), (A.ARRAY (A.FLOAT)))
      | A.BOOL ->
