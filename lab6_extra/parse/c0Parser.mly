...
%token BOOL VOID CHAR STRING FLOAT
%token SEMI QUESTIONMARK COLON COMA MAIN F
%token DOT RARROW
%token <Int32.t> DECCONST
%token <Int32.t> HEXCONST
%token <Base.Float.t> FCONST
...
sdecl :
  ...
  | STRUCT F SEMI               { A.SDecl (Symbol.symbol "f") }
  ;
...
sdef :
  ...
  | STRUCT F LBRACE fieldlist RBRACE SEMI { A.SDef (Symbol.symbol "f", $4) }
  ;
...
field :
  ...
  | types F SEMI             { A.Param (Symbol.symbol "f", $1) }
  | bigtype F SEMI           { A.Param (Symbol.symbol "f", $1) }
  ;
...
bigtype :
  ...
  | STRUCT F                 { A.STRUCT (Symbol.symbol "f") }
  ;

stmts :
  /* empty */                   { A.Nop }
 | decl SEMI stmts              { marks (A.Declare ($1, $3)) (1, 3) }
 | stmt stmts                   { stmts_helper $1 $2 }
 ;
...
decl :
 ...
 | types F                     { A.NewVar (Symbol.symbol "f", $1) }
 | types F ASSIGN exp          { A.Init (Symbol.symbol "f", $1, $4) }
 ;
...
floatconst :
  FCONST              { A.FConstExp $1 }
  | DECCONST F        { A.FConstExp (Int32.to_float $1) }
