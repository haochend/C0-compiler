...
let rec tc_exp env func_env type_env s_env ast ext t =
  ...
  | A.FConstExp f ->
    begin
      match t with
      | Some A.FLOAT -> (Some A.FLOAT, func_env)
      | None -> (Some A.FLOAT, func_env)
      | _ -> ErrorMsg.error ext
               ("expecting a float, but something else was given");
        raise ErrorMsg.Error
    end
  | A.BoolExp _ ->
    begin
    ...
    begin
      ...
      | A.NEGATIVE ->
        (match t with
         ...
         | Some A.FLOAT ->
           let (_, func_env) = tc_exp env func_env type_env s_env e ext (Some A.FLOAT)
           in (Some A.FLOAT, func_env)
         | None -> let (t', func_env) = tc_exp env func_env type_env s_env e ext None
           in (match t' with
               | Some A.INT -> (t', func_env)
               | Some A.FLOAT -> (t', func_env)
               | _ -> ErrorMsg.error ext
                        ("expecting an int or a float, but some other type was given");
                 raise ErrorMsg.Error)
         | _ -> ErrorMsg.error ext
                       ("expecting an int or a float, but some other type was given");
           raise ErrorMsg.Error)
  ...
  | A.Binop (oper, e1, e2) ->
    begin
      ...
      | A.LESSTHAN ->
        (match t with
         | Some A.BOOL -> let (t1, func_env) = tc_exp env func_env type_env s_env e1 ext None
           in let (t2, func_env) = tc_exp env func_env type_env s_env e2 ext None
           in (match t1, t2 with
               | Some A.INT, Some A.INT -> (Some A.BOOL, func_env)
               | Some A.FLOAT, Some A.FLOAT -> (Some A.BOOL, func_env)
               | _, _ -> ErrorMsg.error ext
                   ("types between two sides of the comparison should match\n");
                 raise ErrorMsg.Error)
         | None -> let (t1, func_env) = tc_exp env func_env type_env s_env e1 ext None
           in let (t2, func_env) = tc_exp env func_env type_env s_env e2 ext None
           in (match t1, t2 with
               | Some A.INT, Some A.INT -> (Some A.BOOL, func_env)
               | Some A.FLOAT, Some A.FLOAT -> (Some A.BOOL, func_env)
      ...
      | _ ->
        (match t with
         | Some A.INT -> let (_, func_env) = tc_exp env func_env type_env s_env e1 ext (Some A.INT)
           in let (_, func_env) = tc_exp env func_env type_env s_env e2 ext (Some A.INT)
           in (Some A.INT, func_env)
         | Some A.FLOAT -> let (_, func_env) = tc_exp env func_env type_env s_env e1 ext (Some A.FLOAT)
           in let (_, func_env) = tc_exp env func_env type_env s_env e2 ext (Some A.FLOAT)
           in (Some A.FLOAT, func_env)
         | None -> let (t1, func_env) = tc_exp env func_env type_env s_env e1 ext None
           in let (t2, func_env) = tc_exp env func_env type_env s_env e2 ext None
           in (match t1, t2 with
               | Some A.INT, Some A.INT -> (Some A.INT, func_env)
               | Some A.FLOAT, Some A.FLOAT -> (Some A.FLOAT, func_env)
               | _, _ -> ErrorMsg.error ext
                           ("two sides of the binop don't match\n");
                 raise ErrorMsg.Error)
         | _ -> ErrorMsg.error ext
                       ("expecting an int, but some other type was given");
           raise ErrorMsg.Error)
