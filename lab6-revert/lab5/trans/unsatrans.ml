...
let trans_gdecl func_env struct_env struct_detail_env struct_size_env typedef_env temp_cnt_list safe_mode= function
  | A.Main (typedef, stm) ->
    begin
      Temp.reset();
      Arguement.reset();
      ...
      begin
        ...
        | _ ->
          begin
            if safe_mode then
              begin
                ([[(T.FUNCDEF (Symbol.name id, temp_cnt))]
                  @argltrue
                  @anotherl],func_env,struct_env,struct_detail_env,struct_size_env,typedef_env,new_temp_cnt_list)
              end
              ...
              begin
                match Symbol.name id with
                | "howManyBits" ->([[T.FUNCTIONCALL (Symbol.name id)]],func_env,struct_env,struct_detail_env,struct_size_env,typedef_env,new_temp_cnt_list)
...
let rec translate_gdecl gdecls func_env struct_env struct_detail_env struct_size_env typedef_env temp_cnt_list safe_mode=
  begin
    match gdecls with
    ...
    | Some gdecls ->
      (match gdecls with
       ...
       | gdecl::right ->
         let (stms1, func_env1,struct_env1,struct_detail_env1,struct_size_env1,typedef_env1,temp_cnt_list1) = (trans_gdecl func_env struct_env struct_detail_env struct_size_env typedef_env temp_cnt_list safe_mode gdecl) in
         let (stms2, func_env2,struct_env2,struct_detail_env2,struct_size_env2,typedef_env2,temp_cnt_list2) = (translate_gdecl (Some right) func_env1 struct_env1 struct_detail_env1 struct_size_env1 typedef_env1 temp_cnt_list1 safe_mode) in
