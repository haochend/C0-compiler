    ...
    begin
      ...
      | A.NewVar (id, typedef) ->
        ...
        begin
          ...
          let following = trans_stms env' func_env struct_env struct_size_env arr_env type_env' typedef_env struct_detail_env func_id inline_env safe_mode arg_temp_list l0 opt_reg stm in
          match typedef with
          begin
            | A.POINTER ptype->
              (following,env, arr_env,type_env)
            | A.ARRAY atype ->
              (following,env, arr_env,type_env)
            | A.STRUCT stype ->
              (following,env, arr_env,type_env)
