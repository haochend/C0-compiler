...
let main cmd =
  try
    ...
    let (ir,func_env,struct_env,struct_detail_env,struct_size_env,_,temp_cnt_list,inline_env) =
      begin
        match cmd.opt_lvl with
        | Opt_None ->
          Trans.translate_gdecl (Some ast) init_func_env init_struct_env init_struct_detail_env init_struct_size_env init_typedef_env [] cmd.safe_mode S.empty false
        | Opt_Regalloc ->
          Trans.translate_gdecl (Some ast) init_func_env init_struct_env init_struct_detail_env init_struct_size_env init_typedef_env [] cmd.safe_mode S.empty true
