  ...
  try
    ...
    | X86_64 ->
      begin
        ...
        let (ir,func_env,struct_env,struct_detail_env,struct_size_env,_,temp_cnt_list,inline_env) =
          Trans.translate_gdecl (Some ast) init_func_env init_struct_env init_struct_detail_env init_struct_size_env init_typedef_env [] cmd.safe_mode S.empty in
    ...
    | LLVM ->
      begin
        ...
        say_if cmd.verbose (fun () -> "Translating...");
        let (ir,func_env,struct_env,struct_detail_env,struct_size_env,_,temp_cnt_list,inline_env, dec_func_env) =
...
(*
        ...
        let code = (match cmd.safe_mode with
            ...
            | _ -> String.concat (List.map full ~f:Llvmreadable.format)) in
        ...
        let comm = "opt -S -mem2reg " ^ pfname ^ " -o " ^ pfname in
        (* Done *)
