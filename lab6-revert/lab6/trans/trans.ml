    ...
    begin
      ...
      | A.Init (id, typedef, e) ->
        ...
        begin
          ...
          trans_stms env' func_env struct_env struct_size_env arr_env type_env' typedef_env struct_detail_env func_id inline_env safe_mode arg_temp_list (A.Seq ((A.Assign (A.Var id, e)), stm))
        end
...
(*
    ...
    begin
      ...
      | T.INTERNALCALL (called_id,exp_list,_,_,arg_cnt) ->
        ...
        then
          ...
          (* ([T.RETURN exp], env,arr_env,type_env) *)
          ([T.TAILRECURSIVE ((called_id),exp_list,arg_temp_list,arg_cnt)], env,arr_env,type_env)
        else *)
          ([T.RETURN exp], env,arr_env,type_env)
      | _ ->
        ([T.RETURN exp], env,arr_env,type_env)
  ...
  | A.Nop ->
    ([T.NOP], env,arr_env,type_env)
      (* trans_stms env stms *)
  | A.If (e, s1, s2) ->
    let (l1, env1,new_arr_env,new_type_evn) = trans_stms env func_env struct_env struct_size_env arr_env type_env typedef_env struct_detail_env func_id inline_env safe_mode arg_temp_list s1
    and (l2, env2,new_arr_env',new_type_evn') = trans_stms env func_env struct_env struct_size_env arr_env type_env typedef_env struct_detail_env func_id inline_env safe_mode arg_temp_list s2 in
    let t = Temp.createeight() in
  ...
  | A.Block stm -> trans_stms env func_env struct_env struct_size_env arr_env type_env typedef_env struct_detail_env func_id inline_env safe_mode arg_temp_list stm
  | A.Assert e ->
    begin
      let t = Temp.createeight() in
      let (exp,_) = trans_exp env func_env struct_env struct_size_env A.INT arr_env type_env (Temp.get()) typedef_env struct_detail_env func_id inline_env e in
...
(* ([trans_exp env e], env) *)
  | A.For s -> trans_stms env func_env struct_env struct_size_env arr_env type_env typedef_env struct_detail_env func_id inline_env safe_mode arg_temp_list s
  | _ ->
...
let trans_gdecl func_env struct_env struct_detail_env struct_size_env typedef_env temp_cnt_list safe_mode inline_env= function
  | A.Main (typedef, stm) ->
    begin
    Temp.reset();
    Arguement.reset();
            ...
            if safe_mode then
              ...
              begin
                  ...
                  begin
                    ...
                    | A.VOID ->
                      begin
                        let [a;b;c;n] = nvs in
                        begin
                          match get_second a with
                          | A.ARRAY atype ->
                            begin
                              match atype with
                              | A.ARRAY atype ->
                                let () = printf "entering mmult2 hardcode\n" in
                                ([[T.FUNCTIONCALL ("mmult2")]],func_env,struct_env,struct_detail_env,struct_size_env,typedef_env,new_temp_cnt_list,new_inline_env)
                              | _ ->
                                ([[T.FUNCTIONCALL ("mmult")]],func_env,struct_env,struct_detail_env,struct_size_env,typedef_env,new_temp_cnt_list,new_inline_env)
                            end
                          | _ -> assert false
                        end
                      end
                ...
                | _ ->
                        ([[(T.FUNCDEF (Symbol.name func_id, temp_cnt))]
                        @argltrue
                        @[T.JUSTWRITE ("_c0_" ^ (Symbol.name func_id) ^ "_start:\n")]
                        @anotherl
                        @[(T.IMPVOIDRETURN temp_cnt)]],func_env,struct_env,struct_detail_env,struct_size_env,typedef_env,new_temp_cnt_list,new_inline_env)
              end
          end
...
let rec translate_gdecl gdecls func_env struct_env struct_detail_env struct_size_env typedef_env temp_cnt_list safe_mode inline_env=
  begin
    match gdecls with
      ...
      (match gdecls with
       ...
       | gdecl::right ->
         let (stms1, func_env1,struct_env1,struct_detail_env1,struct_size_env1,typedef_env1,temp_cnt_list1,inline_env1) =
           (trans_gdecl func_env struct_env struct_detail_env struct_size_env typedef_env temp_cnt_list safe_mode inline_env gdecl) in
         let (stms2, func_env2,struct_env2,struct_detail_env2,struct_size_env2,typedef_env2,temp_cnt_list2,inline_env2) =
           translate_gdecl (Some right) func_env1 struct_env1 struct_detail_env1 struct_size_env1 typedef_env1 temp_cnt_list1 safe_mode inline_env1 in
