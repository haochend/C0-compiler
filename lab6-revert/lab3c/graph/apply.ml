	...
	List.map ~f:(fun ass ->
      ...
      | AS.UNOP (oper, s1, s2) ->
        begin
          match (s1, s2) with
          | AS.TEMP t1, AS.TEMP t2 ->
            AS.UNOP (oper, AS.TEMP (find_color t1 coloring), AS.TEMP (find_color t2 coloring))
          | t1, AS.TEMP t2 ->
            AS.UNOP (oper, t1, AS.TEMP (find_color t2 coloring))
          | AS.TEMP t1, t2 ->
            AS.UNOP (oper, AS.TEMP (find_color t1 coloring), t2)
          | _, _ -> AS.UNOP (oper, s1, s2)
        end
      | AS.CP (s, l1, l2) ->
        begin
          match s with
          | AS.TEMP t ->
            AS.CP (AS.TEMP (find_color t coloring), l1, l2)
          | _ -> AS.CP (s, l1, l2)
        end
      | AS.GOTO l -> AS.GOTO l
      | AS.WRITELABEL l -> AS.WRITELABEL l
	) assem
